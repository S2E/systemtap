...
set<interned_string>
get_buildids(string process_path){
  ...
  char* raw_metadata;
  #ifdef HAVE_FIND_METADATA
    if(0 != debuginfod_find_metadata(client.get(), process_path.c_str(), &raw_metadata) || 0 == strcmp(raw_metadata, "[\n]"))
      throw SEMANTIC_ERROR(_("can't retrieve buildids from debuginfod"));
  #else
    throw SEMANTIC_ERROR(_F("can't retrieve buildids for %s from debuginfod. Invalid elfutils version", process_path.c_str()));
  #endif

  vector<pid_t> debuginfod_workers;
  json_object *metadata_array = json_tokener_parse(raw_metadata);
  if(!metadata_array)
  ...
  string buildid;
  for(size_t i = 0, n = json_object_array_length(metadata_array); i < n; i++)
  {
    ...
    if(is_build_id(buildid) && buildids.find(buildid_is) == buildids.end() &&
        json_object_object_get_ex(file_metadata, "atype", &json_field) && 0 == strcmp(json_object_get_string(json_field), "e"))
    {
  ...
  vector<derived_probe *> & finished_results)
{
  ...
  set<interned_string> buildids = get_buildids(process_path);
